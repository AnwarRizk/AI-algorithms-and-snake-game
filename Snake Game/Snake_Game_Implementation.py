...
# Spot Class to initialize some properties for each index in the grid 
class Spot:
    ...
    def show(self, color):
        ...
        # rect, circle, ellipse, arc, polygon, line, lines, aaline, aalines, aapolygon, aarect, aacircle, aapolygon
        draw.polygon(screen, color, [(self.x*widthOfCell, self.y*heightOfCell), (self.x*widthOfCell+widthOfCell, self.y*heightOfCell), (self.x*widthOfCell+widthOfCell, self.y*heightOfCell+heightOfCell), (self.x*widthOfCell, self.y*heightOfCell+heightOfCell)])
...
food_array = [food]
while not done:
    clock.tick(6)          # sets the maximum FPS
    screen.fill(NORD)      # fills the screen with the color NORD
